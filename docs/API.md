...
**갱신 토큰을 만들지 않는 이유** — refresh 토큰은 저장·회전·폐기 설계가 딸려온다.
...
**엔드포인트 22개.** 지금 `api.ts` 에 있는 것은 이 중 **읽기 7개**뿐이다.
...
| # | 항목 | 지금 이 문서의 값 | 걸리는 점 |
|---|---|---|---|
| 1 | 토큰 만료 | 14일 · 갱신 없음 | 더 짧게 하려면 refresh 설계가 딸려온다 |
| 2 | 모임 삭제 | **없다** | H0에 삭제 버튼이 없다. 잘못 만든 모임을 어떻게 치우나 |
| 3 | 본인 탈퇴 | **없다** | 참여자가 스스로 빠지는 경로 (SPEC 갱신 목록에 있음) |
| 4 | `429` 임계값 | 미정 | `GET /g/{token}` 에 IP당 몇 회로 둘지 |
| 5 | OG 응답 | 미정 | `/g/{token}` 을 백엔드가 HTML로도 서빙해야 한다 (`ADR-007`) |
