...
**상태** ⚠️ **`ADR-013`으로 대체됨(2026-08-21)** · **관련** `SPEC.md` §10

> **이 문서는 역사적 기록으로 남긴다.** 도메인이 Group/Meeting + 실시간 채팅으로
> 확장되면서 "모놀리스라 K8s가 필요 없다"는 이 문서의 전제가 깨졌다. 지금 배포
> 방식은 `ADR-013`(Spring Cloud + k3s)을 따른다. **아래 본문의 이유(운영 대상을
> 늘리지 않는다, K8s 학습곡선 비용)는 여전히 유효한 논증**이고 `ADR-013`이 그
> 우려에 어떻게 답했는지(서비스 3개로 제한, Eureka/Config Server 배제) 참고할 것.
