...
**상태** ✅ **유효 — MVP 배포 방식** · **관련** `ADR-014`, `ADR-013`, `SPEC.md` §10

> **이력:** 2026-08-21 `ADR-013`(MSA + k3s)이 이 문서를 대체했으나,
> 2026-08-23 `ADR-014`로 **되돌렸다.** `013`이 구조를 올린 근거가 "실제 필요"가
> 아니라 "학습 목적"이었기 때문이다. MVP는 이 문서대로 **단일 VM + docker compose**로
> 간다. `ADR-013`은 폐기가 아니라 **보류**이며, `ADR-014`의 재검토 조건이 충족되면
> 발동한다. 아래 본문(운영 대상을 늘리지 않는다, K8s 학습곡선 비용)이 그대로 유효하다.
