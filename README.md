...
## 참여자는 웹, 주최자는 앱 — 두 플랫폼을 함께 만든다

같은 모임 안에서도 **입장이 다르면 마찰의 크기가 다르다.**

| | 참여자 | 주최자 |
|---|---|---|
| 인원 | 모임마다 여러 명, 대부분 낯선 카톡 링크로 진입 | 모임마다 한 명, 매번 다시 쓰는 사람 |
| 요구 | 설치 없이 3초 안에 체크만 끝내면 됨 | 차수 편집·명단 관리·정산 확정 등 반복 사용 |
| 선택한 플랫폼 | **웹** (React · Vercel) | **앱** (Kotlin Multiplatform) |

**전원에게 앱 설치를 요구하는 순간 이 서비스는 성립하지 않는다.** 링크를 받은
사람 중 상당수가 설치 단계에서 이탈하고, 그러면 총무가 결국 대신 입력하게 된다
(`ADR-003`). 그래서 참여자는 웹에 남기고, **반복해서 쓰는 주최자에게만** 앱의
깊이(위젯, 오프라인 편집, 푸시 알림)를 준다.

**두 플랫폼은 순서대로가 아니라 나란히 만든다.** 서로 다른 코드베이스가 같은
계약(`docs/API.md`)만 보고 각자의 속도로 움직인다 — 참여자 웹이 먼저 끝났다고
주최자 앱을 기다릴 필요가 없고, 반대도 마찬가지다. 계약이 먼저 확정되어 있기
때문에 가능한 일이다.

| | 상태 |
|---|---|
| 참여자 웹 | 화면 12개 구현 · 실제 도메인에 배포 (백엔드 연동 전이라 목 데이터로 동작) |
| 주최자 앱 | Kotlin Multiplatform · Android 우선 — 설계 완료, 구현 예정 |

`core` 계산 엔진이 Kotlin이라 **앱과 서버가 같은 언어·같은 계산 모듈을 공유한다**
— 앱에서도 서버와 동일한 계산 결과를 재현할 수 있다(오프라인 미리보기 등에 활용 가능).
